...
CREATE TABLE `__new_pickup_order` (
...
INSERT INTO `__new_pickup_order`("id", "booking_opens", "customer_id", "pickup_occasion_id", "status_id") SELECT "id", "booking_opens", "customer_id", "pickup_occasion_id", "status_id" FROM `pickup_order`;
DROP TABLE `pickup_order`;
ALTER TABLE `__new_pickup_order` RENAME TO `pickup_order`;
